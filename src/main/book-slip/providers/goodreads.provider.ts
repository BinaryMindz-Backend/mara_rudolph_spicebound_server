...
export class GoodreadsProvider {
    ...
    async getRatings(
        ...
        try {
            ...
            if (!response.ok) {
                this.logger.warn(`⚠️ Goodreads search failed with status: ${response.status}`);
                return null;
            }
            ...
            if (averageRating === undefined || ratingsCount === undefined) {
                this.logger.warn(`⚠️ Could not find valid non-zero rating string on Goodreads for ${query}`);
                return null;
            }
